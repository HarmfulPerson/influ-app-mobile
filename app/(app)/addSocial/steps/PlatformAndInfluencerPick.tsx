...
export type Platform = { name: string; numberOfPublications: number };
...
const PlatformAndInfluencerPick = () => {
  ...
  const [platformAndInfluencer, setPlatformAndInfluencer] = useState<PlatformAndInfluencer>({ platforms: [], influencer: [] });
  const [isOpenPlatformModal, setIsOpenPlatformModal] = useState<boolean>(false);
  ...
  const canProceed = () => platformAndInfluencer.platforms.length && platformAndInfluencer.influencer.length;
  ...
  const mainArea = (
    <View>
      ...
      <View style={styles.pickPlatformContainer}>
        ...
        <Text style={styles.subtitle}>Wybierz platforme dla współpracy</Text>
        <ModalMechanism
          open={isOpenPlatformModal}
          setOpen={setIsOpenPlatformModal}
          triggerButton={
            <TamaguiButton style={styles.searchButton} borderWidth={1} onPress={openModalPlatform} borderColor={Colors.grayscale.surface.default}>
              <Input
                borderWidth={0}
                placeholder="Kliknij, aby wybrać platformę"
                style={{ flex: 1 }}
                value={platformAndInfluencer.platforms.map((eachPlatform: Platform) => eachPlatform.name).join(", ")}
          ...
          children={<DropdownBottom closeModal={closeModalPlatform} children={<PlatformPicker setPlatformAndInfluencer={setPlatformAndInfluencer} platformAndInfluencer={platformAndInfluencer} closeModalPlatform={closeModalPlatform} />} />}
        />
      </View>
      <View style={styles.pickInfluencerContainer}>
        <Text style={styles.title}>Wybierz twórcę</Text>
        <Text style={styles.subtitle}>Wybierz twórcę z którym chcesz współpracować</Text>
        <ModalMechanism
          open={isOpenInfluencerModal}
          setOpen={setIsOpenInfluencerModal}
          triggerButton={
            <TamaguiButton style={styles.searchButton} borderWidth={1} onPress={openModalInfluencer} borderColor={Colors.grayscale.surface.default}>
              <Input
                borderWidth={0}
                placeholder="Wyszukaj twórcę"
                style={{ flex: 1, color: "white" }}
                value={platformAndInfluencer.influencer[0]?.username}
              ...
              {!!platformAndInfluencer.influencer.length && (
                <View onPress={handleClearInfluencer} style={styles.searchIconContainer}>
                  <X width={12} height={12} color={Colors.grayscale.surface.darker} />
                </View>
              )}
            </TamaguiButton>
          }
          children={<DropdownBottom closeModal={closeModalInfluencer} children={<InfluencerPicker displayEmptyListComponent={false} handleButtonClick={handleButtonClick} invitedInfluencers={platformAndInfluencer.influencer.length ? [platformAndInfluencer.influencer[0].uid] : []} />} />}
        />
      </View>
    </View>
  );
  const botttomArea = (
    <View style={styles.navitionButtonsContainer}>
      ...
      <Button variant="primary" disabled={!canProceed()} onPress={handleNavigateNextPage} style={styles.navigationButton} text="Dalej" />
    </View>
  );
