...
export default function CooperationList() {
    const {
        ...
        rowsPerPage: rowsPerPageCampaigns,
    } = useFilter();
    const {
        page: socialsPage,
        updatePage: updateSocialsPage,
        rowsPerPage: rowsPerPageSocials,
    } = useFilter();
    ...
    const handleFetchData = async () => {
        const campaigns = await handleFetchCampaignOrSocial<Campaign>(
            URL.campaigns,
            campaignsPage,
            rowsPerPageCampaigns
        );
        const socials = await handleFetchCampaignOrSocial<Social>(
            URL.social,
            socialsPage,
            rowsPerPageSocials
        );
        ...
        setDisplayData([
            ...campaigns.data.data.rows,
            ...socials.data.data.rows,
        ]);
        setCampaignsCount(campaigns.data.data.count);
    ...
    const handleReachEndCampaigns = async (): Promise<boolean> => {
        ...
        const response: any = await handleFetchCampaignOrSocial<Campaign>(
            URL.campaigns,
            campaignsPage + 1,
            rowsPerPageCampaigns
        );
    ...
    const handleReachEndSocials = async (): Promise<boolean> => {
        ...
        const response: any = await handleFetchCampaignOrSocial<Social>(
            URL.social,
            socialsPage + 1,
            rowsPerPageSocials
        );
    ...
    async function handleFetchCampaignOrSocial<T>(
        url: string,
        pageToFetch: number,
        rowsPerPage: number
    ): Promise<AxiosResponse> {
        return fetchData<RequestData<{ count: number; rows: Array<T> }>>(
            `${url}?${parseObjectToUrlParams({
                ...stringifyFilterData(),
