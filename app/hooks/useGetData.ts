...
export const getData = async <T>(url: string, options?: AxiosRequestConfig): Promise<T | void> => {
  try {
    const response: AxiosResponse = await axios(`http://192.168.100.158:4000/api/v1${url}`, {
      ...options,
      headers: {
        ...options?.headers,
      },
  ...
  const fetchData = async <T>(url: string, options?: AxiosRequestConfig): Promise<AxiosResponse<T>> => {
    ...
    try {
      const response: AxiosResponse = await axios(`http://192.168.100.158:4000/api/v1${url}`, {
        ...
        headers: {
          ...options?.headers,
          Authorization: `Bearer ${session?.data.tokens.token ? session.data.tokens.token : 1}`,
        },
      });
      setIsLoading(false);
      return response;
