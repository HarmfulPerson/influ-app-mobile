...
export default function ChoosePhoto() {
  ...
  const handleNavigateWithPhoto = async () => {
    const response = await FileSystem.uploadAsync(`http://192.168.100.158:4000/api/v1/file/userAvatar/${session.data.userData.uid}`, image as string, {
      headers: {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.contentWrapper}>
        <View style={{ flex: 1 }}>
          <Text style={styles.titleText}>Wybierz zdjecie profilowe</Text>
          <Text style={styles.infoText}>Posiadasz logo firmy? prześlij je teraz!</Text>
          <View style={styles.centralAreaContainer}>
            <View style={styles.addPhotoContainer} onPress={() => pickImage()}>
              {image ? <Image source={{ uri: image }} style={styles.pickedImage} /> : <SuseuSvg width="80%" height="80%" fill={Colors.grayscale.surface.darker} />}
              {!image && (
                <View style={styles.plusSignContainer}>
                  <Plus width={24} height={24} color={Colors.primary.border.default} />
                </View>
              )}
            </View>
          </View>
        ...
        <View style={styles.buttonsContainer}>
          <Button style={styles.navigateNextButton} variant="primary" disabled={!image} text="Dalej" onPress={handleNavigateWithPhoto} />
          <Button variant="secondary" style={styles.skipButton} text="Pomiń" onPress={naviateToNotificationsPage} />
        </View>
      </View>
    </View>
  );
