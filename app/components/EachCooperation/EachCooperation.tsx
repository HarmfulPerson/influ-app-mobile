import { Text, View } from "tamagui";
import { ImageBackground } from "react-native";
import SocialIcon from "../SocialIconParser";
import { styles } from "./styles";
import type { ViewStyle } from "react-native";
import { Social } from "../../types/social";
import { router } from "expo-router";

export type EachCooperationProps = {
  item: Social;
  containerStyle?: ViewStyle;
};

const EachCooperation = ({ item, containerStyle }: EachCooperationProps) => {
  return (
    <View
      onPress={() =>
        router.push({
          pathname: "social/socialInfo",
          params: { socialUid: item.uid },
        })
      }
      style={[styles.container, containerStyle]}
    >
      <View style={styles.rowWrapper}>
        <View style={styles.socialIconContainer}>
          <SocialIcon coloured width={32} height={32} social={item.platform} />
        </View>
        <View style={styles.userContainer}>
          <View style={{ marginLeft: 8 }}>
            <View style={styles.userImageContainer}>
              <ImageBackground
                style={styles.userPhoto}
                source={require("../../../assets/images/menu-user-gopher.png")}
              />
            </View>
          </View>
          <View style={styles.userInfoContainer}>
            <Text style={styles.username}>{item.executor.nameOfCompany}</Text>
            <Text style={styles.cooperationTitle}>{item.title}</Text>
            <View style={{ flexDirection: "row" }}>
              {/* <View style={styles.ugcContainer}>
                                <Text style={styles.ugcText}>UGC</Text>
                            </View> */}
              {item.socialStatus?.status && item.socialStatus.status === "finished" && (
                <View style={styles.finishedContainer}>
                  <Text style={styles.finishedText}>ZAKOŃCZONA</Text>
                </View>
              )}
            </View>
          </View>
        </View>
      </View>
    </View>
  );
};

export default EachCooperation;
